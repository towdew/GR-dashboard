...
(function () {
    ...
    Highcharts.chart("chartStatusDonut", {
      ...
      tooltip: {
        pointFormatter: function () {
          return `<span style="color:${this.color}">\u25CF</span> <b>${this.name}</b><br/>비율: <b>${Highcharts.numberFormat(this.y, 1)}%</b><br/>건수: <b>${this.raw.toLocaleString()}</b>`;
        },
      },
      ...
      plotOptions: {
        pie: {
          ...
          dataLabels: {
            enabled: true,
            formatter: function () { if (this.y < 3) return null; return `${this.point.name}: ${Highcharts.numberFormat(this.y, 1)}%`; },
            style: { color: "#e6e8ec", textOutline: "none" },
          },
          showInLegend: true,
          point: {
            events: {
              click: function () {
                selectedStatus = selectedStatus === this.name ? null : this.name;
                if (selectedStatus) {
                  selStatusTxt.textContent = selectedStatus;
  ...
  function renderTitleStatusChart() {
    ...
    Highcharts.chart('chartTitleStatus', {
      ...
      yAxis: {
        min: 0,
        title: { text: '비율(%)', align:'high' },
        labels: { formatter(){ return this.value + '%'; }, style:{ color:'#cbd5e1' } },
      ...
      credits: { enabled:false },
      exporting: { enabled:false },
      series: statuses.map(g => ({
        name: g,
        color: palette[g],
        data: seriesByGroup[g].map((y, idx) => makePoint(g, idx, y))
      }))
    });
  }
